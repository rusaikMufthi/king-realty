...
export default function ClientRegisterPage() {
  ...
  const handleEmailRegister = (e: React.FormEvent) => {
    ...
    setLoading(true);
    setTimeout(() => {
      registerWithEmail(name, email, phone);
      router.push('/client/dashboard');
    }, 600);
  };
      ...
      <div className="gold-card p-8 sm:p-10 rounded-3xl max-w-md w-full border border-[#D4AF37]/40 space-y-6 shadow-2xl relative overflow-hidden">
        ...
        <div className="text-center space-y-2">
          <div className="relative w-12 h-12 rounded-xl bg-[#18181B] border border-[#D4AF37]/40 flex items-center justify-center mx-auto">
            <Image src="/logo.png" alt="KING Realty" width={44} height={44} className="object-contain" />
          </div>
          <span className="text-[10px] text-[#D4AF37] uppercase font-bold tracking-widest font-serif block">
            Exclusive Client Access
          </span>
          <h1 className="text-2xl font-bold font-serif text-slate-100">Create Client Account</h1>
          <p className="text-xs text-slate-400">
            Register to save wishlist properties, schedule private viewings, and track inquiries.
          </p>
        </div>
        ...
        >
          ...
          <span>Register with Google OAuth</span>
        </button>

        <div className="flex items-center gap-3 text-slate-600 text-xs my-2">
          <div className="h-px bg-slate-800 flex-1" />
          <span>or fill registration details</span>
          <div className="h-px bg-slate-800 flex-1" />
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                type="text"
                required
                placeholder="e.g. Mr. Kanishka Perera"
                value={name}
